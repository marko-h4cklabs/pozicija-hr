...
export async function POST(req: NextRequest) {
  ...
  console.log('[generate-whatsapp] apiKey present:', !!apiKey);
  if (!apiKey) {
    return NextResponse.json({ error: 'ANTHROPIC_API_KEY nije konfiguriran.' }, { status: 500 });
  }
  ...
  try {
    ...
    report = body.report;
    if (!report) throw new Error('missing report in request body');
  } catch (e) {
  ...
  const prompt =
    ...
    (ownerFirstName
      ...
      : `- Počni s "Bok,"\n`) +
    `- Rečenica 1: Jedna konkretna činjenica koja zaboli — koristi pravo ime konkurenta i jedan pravi broj (score, recenzije ili procijenjeni gubitak)\n` +
  ...
  console.log('[generate-whatsapp] sending to Anthropic, dataLines:', dataLines);
  ...
  try {
    ...
    console.log('[generate-whatsapp] Anthropic response:', rawText.slice(0, 500));

    let data: Record<string, unknown>;
    try {
      data = JSON.parse(rawText);
    } catch {
      throw new Error(`Anthropic returned non-JSON (status ${res.status}): ${rawText.slice(0, 200)}`);
    }

    if (!res.ok || data.error) {
      const apiErr = data.error as { message?: string; type?: string } | undefined;
      throw new Error(apiErr?.message ?? `Anthropic HTTP ${res.status}`);
    }

    const content = data.content as Array<{ type: string; text: string }> | undefined;
    const message: string = content?.[0]?.text?.trim() ?? '';
    if (!message) throw new Error('Anthropic returned empty message');

    return NextResponse.json({ message });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error('[generate-whatsapp] error:', msg);
